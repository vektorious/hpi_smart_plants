...
![image](img/moisture_circuit_lipo_bb.png)

XIAO ESP32C6 Pinout
<img src="img/XIAO_ESP32C6_pinout.png" alt="XIAO ESP32C6 Pinout"> 


⚠️ Important: Do not connect the li-ion battery yet. We’ll add it only after programming and testing to avoid accidental powering.
...
Unplug the Microcontroller and add the li-ion battery to the circuit (keep it switched off first). Connect the - of the battery holder to the - rail. 

If everything is connected correctly (double check) watch for your data coming in on the dashboard.

If that worked, you are ready for putting everything into the housing. I recommend gluing the the breadboard to the back of the housing by removing parts of the cover of the tape on the back of it. Add the velco tape to the battery holder and the inner side of the housing lid so it can be attached there.

⚠️ Note: Switching the battery connection off and on again, re-triggers a measurement. If there is a Wifi connection, you should see the values appearing on the dashboard. 
